    ...
    for(let i = 0; i < inningsArr.length; i++){
        ...
        if(wTeamName == teamName){
            ...
            /* 
                ...
                2. bowling table
            */
            let tableArr = $(inningsArr[i]).find(".ds-w-full.ds-table.ds-table-xs.ds-table-fixed");
            for(let j = 0; j < tableArr.length; j++){
                let hasClass = $(tableArr[j]).hasClass("ci-scorecard-table");
                // bowling table does not have the above class, hence we can distinguish both table.
                if(!hasClass){
                    let bowlingTable = tableArr[j];
                    let bowlingTableRows = $(bowlingTable).find("tbody .ds-border-b.ds-border-line.ds-text-tight-s");
                    
                    let wicketBowlerMap = {};
                    let maxWicketTaken = 0;

                    for(let k = 0; k < bowlingTableRows.length; k++){
                        let rowElemArr = $(bowlingTableRows[k]).find(".ds-min-w-max");
                        let bowlerName = $(rowElemArr[0]).text();  
                        let bowlerWicket = $(rowElemArr[4]).text();              
                        wicketBowlerMap[bowlerName] = bowlerWicket;
                        maxWicketTaken = Math.max(maxWicketTaken, bowlerWicket);
                    }
                    // console.log(maxWicketTaken);
                    // console.log(wicketBowlerMap);
                    console.log("The highest wicket takers of this game from winning team is/are :")
                    for(let key in wicketBowlerMap){
                        if(wicketBowlerMap[key] == maxWicketTaken){
                            console.log(key);
                        }
                    }
                }                
